...
### Neutral
- **Win Chrome** (`hsl(214 17% 91%)`): the flat workspace the app sits on — like a freshly logged-in Windows desktop.
- **Panel White** (`hsl(0 0% 100%)`): flat desks that hold tables, forms, the cashier's cart table, and inputs.
- **Chrome Ink** (`hsl(210 10% 15%)`): primary text — a blue-grey not-quite-black, exactly as crisp as Win7 rendered it.
- **Hairline Pewter** (`hsl(214 14% 79%)`): 1px borders and row strokes; bank-ledger thin.
- **Muted Field** (`hsl(214 15% 87%)`): hover rows, chips, kbd keys, and washed tool bars — flat fills, no simulated depressions.
- **Control Grey** (`hsl(214 14% 94%)`): the raised grey control (secondary buttons), Win7's neutral chrome.
- **Accent Grey** (`hsl(214 16% 85%)`): hover highlight on ghost controls and rows.
- **Ledger Grey** (`hsl(210 8% 40%)`): secondary text, captions, micro-headers, placeholders.
...
- **Rail Slate** (`hsl(220 16% 14%)`): the `48px` aurora rail's base.
- **Rail Active** (`hsl(211 95% 55%)`): the selected nav tile — Aero Blue, sharp-cornered.
- **Rail Muted / Rail Hover** (`hsl(220 10% 66%)` / `hsl(220 14% 22%)`): idle nav glyphs and their hover fill.
...
### Charts
- **chart-1 … chart-5** (`hsl(211 100% 50%)`, `hsl(280 60% 60%)`, `hsl(160 60% 45%)`, `hsl(28 90% 52%)`, `hsl(2 70% 55%)`): the macOS-derived series for dashboard charts — blue, purple, teal, orange, red.

### Named Rules
**The Sharp-Corner Rule.** Corners are 0–2px. `rounded-md`/`rounded-sm` resolve to 0px and `rounded-lg`+ resolve to 2px in the token math. Rounded-domain interface, rounded cards, and rounded buttons belong to the world this replaces; rectangles read faster and they are the point.
...
**Display/UI Font:** Segoe UI (Windows-native; `'Segoe UI Variable Text'`, `'Segoe UI'`, `-apple-system, BlinkMacSystemFont, system-ui, sans-serif`), with **Segoe UI Light** weight (300) assigned to headline money figures — the Win7 big-number voice, flatted to premium. `--font-headline` maps to `'Segoe UI Variable Display'`.
**Mono Font:** `Consolas`, `'Cascadia Mono'`, `ui-monospace`, `'SF Mono'`, `Menlo`, `monospace` — invoice numbers, voucher codes, reference IDs, and the receipt tape.
...
- **Responsive:** below `sm` the Aurora Taskbar becomes a bottom tab bar (`BottomNav`); the cashier's cart table and payment rail collapse into a single column flow.
...
- **Surfaces (cards, dialogs, panels):** corners `2px` (`rounded-lg`); flat sheets, not puffed cushions. The receipt dialog's paper container uses `rounded-[2px]`.
- **Controls (buttons, inputs, badges, selects):** corners `0px` (`rounded-md` resolves to 0); buttons read as flat tabs. `rounded` (4px) survives only on seated `kbd` key caps.
...
- **Edges:** 1px hairlines everywhere; `hsl(214 14% 79%)` at full opacity for structural separators, 60% on chrome. Sharp enough to cut a receipt.
...
*[Implemented and carbonized from source. Radius and height values are the token-resolved numbers, not Tailwind class names.]*

### Buttons
- **Shape:** rectangles (`0px`, `rounded-md`); `kbd` key caps alone take 4px.
- **Primary (default):** Aero Blue fill, white text, `h-8` (32px) `px-3`, `.gloss-chrome` top shine; hover `bg-primary/90`, active `/80`. The BAYAR action is `h-12` (48px) `text-lg font-black`.
- **Secondary:** Control Grey fill (`hsl(214 14% 94%)`), dark text, hover `/80` — the PARKIR action's chrome.
- **Outline:** Panel White fill, `1px` Hairline Pewter stroke, hover Accent Grey — voucher/discount quick-actions and "Detail".
- **Ghost:** no fill, Accent Grey hover — row icon actions (qty ±, trash).
...
### Inputs / Fields
- **Style:** Panel White fill (`bg-field`), `1px` Hairline stroke, rectangles (`0px`), `h-8` default.
- **Cash input:** `h-9` (36px) `border-2`, Segoe UI bold, `pl-10` with a floating "Rp" prefix, tabular; the counter's primary numeric surface.
- **Focus:** `2px` Aero Blue ring with `2px` offset (`focus-visible:ring-ring`), never a glow.
...
### Badges
- **Style:** `rounded-md` (0px) `px-2 py-0.5`, `12px` medium; solid Aero Blue for default, tinted `/40` fills with dark ink for success/warning/info, solid Ticket Red for destructive. Used for transaction status (PAID / VOIDED).

### Cards / Containers
- **Corner Style:** `2px` (`rounded-lg`).
- **Background:** Panel White on Win Chrome ground; `border` Hairline.
- **Shadow Strategy:** none — flat sheets on the chrome ground (see Elevation).
- **Internal Padding:** `16px` (`p-4`), `space-y-4`.

### Navigation
- **The Aurora Taskbar:** `48px` fixed rail, Rail Slate fill, aurora drift overlay, `1px` top gloss, `border-r` hairline. Nav tiles `36px` square, `2px` corners; the active tile is Aero Blue with a spring-animated shared layout (`layoutId`), idle glyphs Rail Muted warming to white on hover. Tooltips sit right of the rail.
- **Header:** `40px` flat chrome band, `border-b border-border/60`, `backdrop-blur`; holds logo + version, parked carts, Tutup Sif (destructive), notification bell, theme toggle.
- **Mobile:** the rail becomes a bottom tab bar; the cashier's cart + payment columns stack.

### Kbd (shortcut keycap)
- `h-5 min-w-5 px-1.5`, `4px` corners, Hairline border, Muted Field fill, Consolas `10px` semibold, Ledger Grey text — the F1–F8 legend printed along the counter's base.
